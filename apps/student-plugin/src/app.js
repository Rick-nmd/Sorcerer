...
const I18N = {
  en: {
    page_title: "Cooling-Off & Support Companion",
    page_subtitle:
      "Private, explainable support before you decide — with a cooling-off pause and campus alternatives.",
  ...
  zh: {
    page_title: "借贷降温与支持助手",
    page_subtitle: "在你做决定前，提供隐私优先、可解释的陪伴式支持：冷静期、校园替代方案与求助入口。",
    h2_settings: "设置",
    h2_session: "学生会话",
    h2_scenario: "页面检测",
    h2_intervention: "冷静期",
...
function applyStaticUiCopy() {
  ...
  ui.pageTitle.textContent = t("page_title");
...
function renderExplainability({ analysis, selfCheckNotes, costSnapshot }) {
  const notes = [
    ...
    ...selfCheckNotes,
    currentLang === "zh"
      ? `在 APR ${costSnapshot.apr}%、还款期 ${costSnapshot.months} 个月的设定下，预计总还款为 ${t("currency")} ${
          costSnapshot.estimated_total_repayment
        }。`
      : `At ${costSnapshot.apr}% APR and ${costSnapshot.months} months repayment, estimated total repayment is ${t(
          "currency"
        )} ${costSnapshot.estimated_total_repayment}.`
  ];
